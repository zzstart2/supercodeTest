    ...
    <svg width="100%" height="${h}" viewBox="0 0 ${w} ${h}">
      <defs>
        <linearGradient id="area-grad" x1="0" x2="0" y1="0" y2="1">
          <stop offset="0%" stop-color="#E42E2E" stop-opacity="0.25"/>
          <stop offset="100%" stop-color="#E42E2E" stop-opacity="0"/>
        </linearGradient>
      </defs>
      ${yLines}
      <path d="${area}" fill="url(#area-grad)"/>
      <path d="${path}" fill="none" stroke="#E42E2E" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>
      ${pts.map((p, i) => `<circle cx="${p[0]}" cy="${p[1]}" r="3" fill="#fff" stroke="#E42E2E" stroke-width="2"/>`).join('')}
      ${xLabels}
    </svg>
  `;
}
...
function renderBarChart(el, data, opts = {}) {
  ...
  const bars = data.map((d, i) => {
    ...
    return `<rect x="${x}" y="${y}" width="${barW}" height="${barH}" rx="3" fill="#E42E2E" opacity="0.85"/>
